...
class _NutritionistePhoneState extends State<NutritionistePhone> {
  ...
  final Map<String, String> phonePatterns = {
    '+213': r'^(5|6|7)[0-9]{8}$', // Algeria: starts with 5, 6, or 7 followed by 8 digits
    ...
    '+1': r'^[2-9][0-9]{9}$',     // USA/Canada: starts with 2-9 followed by 9 digits
    '+44': r'^[7][0-9]{9}$',      // UK: starts with 7 followed by 9 digits
    '+49': r'^[1-9][0-9]{9,10}$', // Germany: starts with 1-9 followed by 9-10 digits
    ...
    '+966': r'^[5][0-9]{8}$',     // Saudi Arabia: starts with 5 followed by 8 digits
  };
  ...
  void initState() {
    ...
    _phoneController.addListener(() {
      setState(() {
        isButtonEnabled = _phoneController.text.length >= 3 && isValidPhoneNumber(_phoneController.text);
      });
    });
  }
  ...
  int getMaxLengthForSelectedCountry() {
    return phoneMaxLength[selectedCountryCode] ?? 10; // Default to 10 if not specified
  }
  ...
  void _handleContinue() {
    ...
    if ((selectedCountryCode == '+213' || selectedCountryCode == '+33' || selectedCountryCode == '+44') && 
                ...
                children: [
                  ...
                  Container(
                    ...
                    constraints: BoxConstraints(
                      minHeight: screenSize.height - 120, // Account for logo and padding
                    ),
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                                  ...
                                  child: DropdownButton<String>(
                                    ...
                                    onChanged: (value) {
                                      setState(() {
                                        ...
                                        // Reset the text if the current text exceeds the max length for the new country
                                        if (_phoneController.text.length > getMaxLengthForSelectedCountry()) {
                                          _phoneController.text = _phoneController.text.substring(0, getMaxLengthForSelectedCountry());
                                        }
                                      });
                                    },
                                  ),
                                ...
                                Expanded(
                                  child: TextFormField(
                                    ...
                                    decoration: InputDecoration(
                                      ...
                                      counterText: '${_phoneController.text.length}/${getMaxLengthForSelectedCountry()}',
                                    ),
                                    keyboardType: TextInputType.phone,
                                    maxLength: getMaxLengthForSelectedCountry(),
                                    buildCounter: (BuildContext context, {required int currentLength, required bool isFocused, required int? maxLength}) {
                                      return null; // Hide the default counter
                                    },
                                    onChanged: (value) {
                                      // Enforce max length if user pastes longer text
                                      if (value.length > getMaxLengthForSelectedCountry()) {
                                        _phoneController.text = value.substring(0, getMaxLengthForSelectedCountry());
                                        _phoneController.selection = TextSelection.fromPosition(
                                          TextPosition(offset: getMaxLengthForSelectedCountry())
                                        );
                                      }
                                      setState(() {
                                        isButtonEnabled = value.length >= 3 && isValidPhoneNumber(value);
                                      });
                                    },
                                  ),
                                ),
                          ...
                          // Add padding to ensure sufficient space at the bottom
                          SizedBox(height: 260),
                          
                          // Continue button at the bottom
                          Padding(
                            padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom > 0 ? 24 : MediaQuery.of(context).padding.bottom + 16),
                            child: CustomButton(
                              text: 'Continue',
