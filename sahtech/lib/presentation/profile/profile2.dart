...
class _Profile2State extends State<Profile2> {
  ...
  @override
  void dispose() {
    if (widget.userData.preferredLanguage != _translationService.currentLanguageCode) {
  ...
  void _continueToNextScreen() async {
    if (_hasChronicDisease == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(await _translationService.translate('Veuillez sélectionner une option')),
    ...
    if (_hasChronicDisease == true) {
      final result = await Navigator.of(context).push(
        MaterialPageRoute(builder: (context) => Profile3(userData: widget.userData)),
      );

      if (result == 'conditions_selected') {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(await _translationService.translate('Informations enregistrées avec succès!')),
      ...
      Navigator.of(context).push(
        MaterialPageRoute(builder: (context) => Profile4(userData: widget.userData)),
      );
    }
  }
                  ...
                  Padding(
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          Container(
                            width: 1.sw * 0.1 - 3.2.w, // Adjusting width to account for padding
                            height: 4.h,
                            decoration: BoxDecoration(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            FutureBuilder<String>(
                              future: _translationService.translate('Avez vous une maladie chronique ?'),
                              builder: (context, snapshot) {
                                final text = snapshot.data ?? 'Avez vous une maldie chronique ?';
                                return Text(
                                  text,
                                  style: TextStyle(
                                    fontSize: 24.sp,
